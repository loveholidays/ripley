...
func main() {
	...
	disableKeepAlive := flag.Bool("disable-keepalive", false, "Disable HTTP keep-alive (forces new connection per request)")
...
When 0 (default) or negative, reporting is switched off.
  `)
	...
	exitCode = ripley.Replay(*paceStr, *silent, *dryRun, *timeout, *strict, *numWorkers, *connections, *maxConnections, *disableKeepAlive, *printStatsInterval, *metricsServerEnable, *metricsServerAddr)
