...
This repository is onboarded to the loveholidays System Catalog. System catalog files are maintained in `.system-catalog/` and document all systems, APIs, and dependencies in this repository.

**Systems registered:**
- `ripley` - HTTP traffic replay tool
- `linkerdxripley` - Linkerd log converter

**Maintenance:**
- When adding new systems, APIs, or dependencies, update the relevant `.system-catalog/*.yaml` files
- Use the `lh-platform:system-catalog` skill from the [loveholidays marketplace](https://github.com/loveholidays/lh-marketplace)
- Changes are automatically validated on PR and published to the central catalog on merge to main

**Validation:**
- Local validation: `uv run .system-catalog/tools/validate_catalog.py .system-catalog/<system-name>.yaml`
- CI validation runs automatically on PRs that modify `.system-catalog/**/*.yaml` files